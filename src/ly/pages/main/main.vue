...
const F_getTopMenuList = () => {
  getTopMenuList(import.meta.env.VITE_APP_LYSP_NAME).then((res) => {
    //1.获取菜单列表
...
const render = (val) => {
  if (data.defaultActive != val.menuValue) {
    localStorage.setItem("activeTopMenuId_ly", val.menuId);
    localStorage.setItem("activeTopMenuName_ly", val.menuName);
