  ...
  <el-row class="tlw-row">
    <el-col :span="24">
      <el-table :data="tableData" border stripe @row-dblclick="openWorkbox">
        ...
        <el-table-column prop="sqrmc" label="申请单位/个人" width="180" />
        <el-table-column
          prop="spcj"
          label="审批层级"
          width="180"
          :formatter="spqxFormatter"
        />
        <el-table-column prop="pzwh" label="批准文号" width="180" />
        <el-table-column
          prop="pfsj"
          label="批复时间"
          width="180"
        ...
        />
        <el-table-column prop="xzxkh" label="许可证号" width="180" />
      </el-table>
      <pagination
...
import { queryPage } from "@/ly/api/proj/rgfy";
...
const { tableData, districtOptions, pages, queryParams, businessList } =
  toRefs(data);
 ...
 * @param {*} column
 */
const spqxFormatter = (row, column) => {
  var spqx;
  switch (row.spqx) {
    case 1:
      spqx = "国家级";
