...
export function  getRules() {
  var validRules = {
    ...
    jbxxXmqssj:[
      { required: true, message: '项目开始时间不可为空', trigger: 'blur' }, 
    ],
    jbxxXmjssj:[
...
function checkXzgdmj(rule, value, callback) {
  const reg = /(^\d+(\.\d+)?$)/;
  if(!isNullorEmpty(value)){
    if (!reg.test(value)) {
      ...
      if(parseInt(value)<=0){
        return callback(new Error('新增耕地面积应大于0(新增耕地面积 = 新增耕地中水田面积+新增耕地中水浇地面积+新增耕地中旱地面积)'));
      }
    }
  }
  return callback();
