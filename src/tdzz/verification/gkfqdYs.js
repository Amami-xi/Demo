...
export function getRules() {
  var validRules = {
    ...
    zykjNydmj: [
      { required: true, message: '自验扣减农用地面积不可为空', trigger: 'blur' },
      { validator: rules.checkNum, trigger: 'blur' }
    ],
    sbysGdmj: [
    ...
    sdkjxzgdmj: [
      { required: true, message: '实地扣减新增耕地面积不可为空', trigger: 'blur' },
      { validator: rules.checkNum, trigger: 'blur' }
    ],
    sdkjxznydmj: [
      { required: true, message: '实地扣减新增农用地面积不可为空', trigger: 'blur' },
      { validator: rules.checkNum, trigger: 'blur' }
    ],
    lxGdmjhj: [
...
export function checkValidate(context) {
  ...
  let sdkjxzgdmj = isNullorEmpty(context.entity.ys.sdkjxzgdmj) ? 0 : context.entity.ys.sdkjxzgdmj;

  let ghxzNydmj = isNullorEmpty(context.entity.ys.ghxzNydmj) ? 0 : context.entity.ys.ghxzNydmj;
  let zykjNydmj = isNullorEmpty(context.entity.ys.zykjNydmj) ? 0 : context.entity.ys.zykjNydmj;
  let sdkjxznydmj = isNullorEmpty(context.entity.ys.sdkjxznydmj) ? 0 : context.entity.ys.sdkjxznydmj;
  ...
  let xznydbl = (ghxzNydmj - zykjNydmj) / (ghxzNydmj);
  if (xznydbl == 0 || xznydbl < 0.9) {
    flag = false;
    message += "不满足（规划新增农用地面积-县级自验扣减农用地面积)/（规划新增农用地面积）>=90%";
  }
