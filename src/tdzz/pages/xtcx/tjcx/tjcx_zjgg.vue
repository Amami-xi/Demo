  ...
  <el-row class="tlw-row">
    <el-col :span="24">
      <el-table :data="tableData" border stripe v-if="queryParams.hzType === 1" id="table">
        ...
        <el-table-column prop="nxznydmj" label="拟新增农用地面积（公顷）" :formatter="stringToFloat" />
      </el-table>
      <el-table :data="tableData" border stripe v-if="queryParams.hzType === 2" id="table">
        ...
        <el-table-column prop="rknxznydmj" label="入库拟新增农用地面积（公顷）" :formatter="stringToFloat" />
      </el-table>
      <el-table :data="tableData" border stripe v-if="queryParams.hzType === 3" id="table">
        ...
        <el-table-column prop="sqysxznydmj" label="申请验收新增农用地面积（公顷）" :formatter="stringToFloat" />
      </el-table>
      <el-table :data="tableData" border stripe v-if="queryParams.hzType === 4" id="table">
        <el-table-column label="序号" type="index" width="80" />
        <el-table-column prop="xzqmc" label="行政区" width="180" />
        <el-table-column prop="jsgm" label="建设规模" width="180" :formatter="stringToFloat" />
        <el-table-column prop="xmCount" label="项目个数" width="180" :formatter="stringToInt" />
        <el-table-column prop="noxmCount" label="不合格项目个数"  width="180" :formatter="stringToInt" />
        <el-table-column prop="qrxzgdmj" label="确认新增耕地面积（公顷）" :formatter="stringToFloat" />
        <el-table-column prop="sdkjxzgdmj" label="市级核减新增耕地面积（公顷）" width="240" :formatter="stringToFloat" />
        <el-table-column prop="qrxznydmj" label="确认新增农用地面积（公顷）" :formatter="stringToFloat" />
        <el-table-column prop="sdkjxznydmj" label="市级核减新增农用地面积（公顷）" width="240" :formatter="stringToFloat" />
      </el-table>
    </el-col>
  </el-row>
</template>
...
import { getTjcxZjgg } from "@/tdzz/api/xtcx/tjcx";
 ...
 */
const F_init = () => {
  getTjcxZjgg(data.queryParams).then((res) => {
