...
<template>
  ...
  >
    <div class="flow-form">
      ...
      <panel title="复垦前现状地类">
        <el-row>
          <el-col :span="6" :offset="4" width="25%">
      ...
      <panel title="复垦后现状地类">
        <el-row>
          <el-col :span="6" :offset="4" width="25%">
            <el-form-item label="耕地" prop="ys.fkhGdmj" :rules="rules.num">
              <el-input
                :disabled="disabled"
                v-model="entity.ys.fkhGdmj"
              ></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="6" :offset="4">
            <el-form-item label="园地" prop="ys.fkhYdmj" :rules="rules.num">
              <el-input
                :disabled="disabled"
                v-model="entity.ys.fkhYdmj"
              ></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="6" :offset="4" width="25%">
            <el-form-item label="林地" prop="ys.fkhLdmj" :rules="rules.num">
              <el-input
                :disabled="disabled"
                v-model="entity.ys.fkhLdmj"
              ></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="6" :offset="4">
            <el-form-item label="草地" prop="ys.fkhCdmj" :rules="rules.num">
              <el-input
                :disabled="disabled"
                v-model="entity.ys.fkhCdmj"
              ></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="6" :offset="4" width="25%">
            <el-form-item
              label="城镇村及工矿用地"
              prop="ys.fkhCzcjgkydmj"
              :rules="rules.num"
            >
              <el-input
                :disabled="disabled"
                v-model="entity.ys.fkhCzcjgkydmj"
              ></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="6" :offset="4">
        ...
        <el-row>
          <el-col :span="6" :offset="4" width="25%">
            <el-form-item
              label="水域及水利设施用地"
              prop="ys.fkhSyslssydmj"
              :rules="rules.num"
            >
              <el-input
                :disabled="disabled"
                v-model="entity.ys.fkhSyslssydmj"
              ></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="6" :offset="4">
      ...
      <panel title="市级验收扣减">
        <el-row>
          <el-col :span="6" :offset="4">
            <el-form-item
              label="数据复核扣减新增耕地面积"
              prop="ys.sjfhKjxzgdmj"
              :rules="sjyskjDisabled ? null : rules.sjfhKjxzgdmj"
            >
              <el-input
                :disabled="sjyskjDisabled"
                v-model="entity.ys.sjfhKjxzgdmj"
              ></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="16" :offset="4">
            <el-form-item label="扣减内容" prop="ys.sdkjqk">
              <el-input
                type="textarea"
                :disabled="sjyskjDisabled"
...
import { getRules, checkValidate } from "@/tdzz/verification/zbphYs";
import { ref, reactive, toRefs } from "@vue/reactivity";
import { ElMessage } from "element-plus";
import { useRoute } from "vue-router";
import { onMounted, onBeforeUnmount, computed } from "@vue/runtime-core";
import bus from "@/common/utils/bus";
import { init, saveOrUpdate, deleteYs } from "@/tdzz/api/proj/proj-zbph-ys.js";
...
const F_delete = (callback) => {
  deleteYs(data.jbxxSec).then((res) => {
...
onMounted(() => {
  ...
  F_init();
  //2.定义监听事件
  bus.on("zbph_ysxx_save", async (callback) => {
    let saveFlag = await F_save();
    if (callback) {
      callback(saveFlag);
    }
  });
  bus.on("zbph_ysxx_delete", (callback) => {
    F_delete(callback);
  });
});
...
//4.销毁监听事件
onBeforeUnmount(() => {
  bus.off("zbph_ysxx_save");
  bus.off("zbph_ysxx_delete");
});
</script>
