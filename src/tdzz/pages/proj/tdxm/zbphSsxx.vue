  ...
  >
    <panel title="项目基本信息">
      ...
      <el-row>
        ...
        <el-col :span="10" :offset="1">
          <el-form-item label="批复时间" prop="ss.pfsj" :rules="rules.pfrq">
            <el-date-picker
              type="date"
              :formatter="dateFormat"
              :disabled="disabled"
      ...
      <el-row>
        <el-col :span="10" :offset="1">
          <el-form-item label="高标准基本农田建设条件" prop="ss.gbzjstj">
            <el-input
              :disabled="disabled"
      ...
      <el-row>
        ...
        <el-col :span="10" :offset="1">
          <el-form-item
            ...
            :rules="rules.num"
          >
            <el-input :disabled="disabled" v-model="entity.ss.gclGd"></el-input>
          </el-form-item>
        </el-col>
      </el-row>
      <el-row>
...
import { getRules, checkValidate } from "@/tdzz/verification/zbphSs";
import { ref, reactive, toRefs } from "@vue/reactivity";
import { init, saveOrUpdate, deleteSs } from "@/tdzz/api/proj/proj-zbph-ss";
...
const { businessName, catalog, disabled, entity, rules, xzqdm } = toRefs(data);
...
const F_delete = (callback) => {
  deleteSs(data.jbxxSec).then((res) => {
...
onMounted(() => {
  ...
  F_init();
  //2.定义监听事件
  bus.on("zbph_ssxx_save", async (callback) => {
    let saveFlag = await F_save();
    if (callback) {
      callback(saveFlag);
    }
  });
  bus.on("zbph_ssxx_delete", (callback) => {
    F_delete(callback);
  });
});
...
//4.销毁监听事件
onBeforeUnmount(() => {
  bus.off("zbph_ssxx_save");
  bus.off("zbph_ssxx_delete");
});
</script>