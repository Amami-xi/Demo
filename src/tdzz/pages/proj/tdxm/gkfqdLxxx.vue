...
import { getRules, checkValidate } from "@/tdzz/verification/gkfqdLx";
import { ref, reactive, toRefs } from "@vue/reactivity";
import { ElMessage } from "element-plus";
import { useRoute } from "vue-router";
import { onMounted, onBeforeUnmount, computed } from "@vue/runtime-core";
import bus from "@/common/utils/bus";
import { init, saveOrUpdate, deleteLx } from "@/tdzz/api/proj/proj-gkfqd-lx.js";
...
onMounted(() => {
  ...
  F_init();
  //2.定义监听事件
  bus.on("gkfqd_lxxx_save", async (callback) => {
    let saveFlag = await F_save();
    if (callback) {
      callback(saveFlag);
    }
  });
  bus.on("gkfqd_lxxx_delete", (callback) => {
    F_delete(callback);
  });
});
...
//4.销毁监听事件
onBeforeUnmount(() => {
  bus.off("gkfqd_lxxx_save");
  bus.off("gkfqd_lxxx_delete");
});
</script>