        ...
        <el-row>
          <el-col :span="6" :offset="4">
            <el-form-item
              ...
              :rules="rules.xzgdmj"
            >
              <el-input disabled v-model="fkhXzgdmj"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
...
import { getRules, checkValidate } from "@/tdzz/verification/zbphLx";
import { ref, reactive, toRefs } from "@vue/reactivity";
import { ElMessage } from "element-plus";
import { useRoute } from "vue-router";
import { onMounted, onBeforeUnmount, computed } from "@vue/runtime-core";
import bus from "@/common/utils/bus";
import { init, saveOrUpdate, deleteLx } from "@/tdzz/api/proj/proj-zbph-lx.js";
...
onMounted(() => {
  ...
  //2.定义监听事件
  //zbph_lxxx ：代表当前页面路由
  bus.on("zbph_lxxx_save", async (callback) => {
    let saveFlag = await F_save();
    if (callback) {
      callback(saveFlag);
    }
  });
  bus.on("zbph_lxxx_delete", (callback) => {
    F_delete(callback);
  });
});
...
//4.销毁监听事件
onBeforeUnmount(() => {
  bus.off("zbph_lxxx_save");
  bus.off("zbph_lxxx_delete");
});
</script>
