<template>
  ...
  >
    <panel title="立项阶段基本信息">
      ...
      <el-row>
        <el-col :span="7" :offset="3">
          <el-form-item label="下达预算计划机关" prop="lx.pfjg"
    ...
    <panel title="计划提质改造">
      <el-row>
        <el-col :span="7" :offset="3">
          <el-form-item label="提质改造面积" prop="lx.tzgzMj" :rules="rules.num"
            ><el-input
              :disabled="disabled"
    ...
    <panel title="计划建设信息">
      <el-row>
        <el-col :span="7" :offset="3">
          <el-form-item label="建设总规模" prop="lx.jsgmZmj" :rules="rules.jsgm"
            ><el-input
              :disabled="disabled"
      ...
      <el-row>
        ...
        <el-col :span="7" :offset="3">
          <el-form-item
            label="新增农用地及未利用地面积"
            prop="lx.jsgmXznydwlyd"
            :rules="rules.num"
            ><el-input
              :disabled="disabled"
              v-model="entity.lx.jsgmXznydwlyd"
            ></el-input> </el-form-item
        ></el-col>
      </el-row>
    </panel>
    <panel title="计划工程量信息">
      <el-row>
        ...
        <el-col :span="7" :offset="3">
          <el-form-item label="土方工程量" prop="lx.gclTfgcl" :rules="rules.num"
            ><el-input
              :disabled="disabled"
...
import { init, saveOrUpdate, deleteLx } from "@/tdzz/api/proj/proj-stbh-lx";
...
const { businessName, catalog, disabled, entity, districtOptions, rules } =
  toRefs(data);
...
onMounted(() => {
  ...
  F_init();
  //2.定义监听事件
  bus.on("stbh_lxxx_save", async (callback) => {
    let saveFlag = await F_save();
    if (callback) {
      callback(saveFlag);
    }
  });
  bus.on("stbh_lxxx_delete", (callback) => {
    F_delete(callback);
  });
});
...
//4.销毁监听事件
onBeforeUnmount(() => {
  bus.off("stbh_lxxx_save");
  bus.off("stbh_lxxx_delete");
});
</script>
