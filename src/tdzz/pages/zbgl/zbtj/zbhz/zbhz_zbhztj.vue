        ...
        <el-row>
            <el-col :span="24">
                <el-form-item label="行政区选择">
                    <el-select v-model="xzqdm" clearable @change="xzqdmChange" @clear="clearXzqdm">
                ...
                <el-form-item label="出入库类型">
                    <el-select v-model="inoutType" clearable @change="typesChange">
                        <el-option
                            v-for="item in inoutTypes"
                            :key="item.value"
                            :label="item.label"
                            :value="item.value"
                        />
                ...
                <el-checkbox v-model="include" label="包含下级行政区指标" size="large" @change="checkChange" />
            </el-col>
        </el-row>
        <el-row>
            <el-col :span="24">
                <el-button type="primary" @click="F_getZbhzmxList">统计</el-button>
            </el-col>
        </el-row>
    </el-form>
    <el-row class="tlw-row">
        <el-col :span="24">
            <el-table :data="tableData" border stripe>
                ...
                <el-table-column prop="czxzqdm" label="行政区代码" />
                <el-table-column prop="gdzb" label="耕地数量（公顷）" />
                <el-table-column prop="stzb" label="水田规模（公顷）" />
                <el-table-column prop="cnzb" label="粮食产能（公顷）" />
            </el-table>
        </el-col>
    </el-row>
</template>
...
import { getTdzzZbhztj } from "@/tdzz/api/zbgl/zbtj/zbhz/zbhz";
...
const F_getZbhzmxList = () => {
    getTdzzZbhztj(data.queryParams).then((res) => {
...
const typesChange = (val) => {
    data.queryParams.inoutType = val;
};
