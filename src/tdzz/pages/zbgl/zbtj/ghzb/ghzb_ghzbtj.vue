<template>
  <el-form :inline="true" class="tlw-search">
    ...
    <el-row>
      <el-col :span="24">
        <el-button type="primary" @click="F_getGhzbtjList">统计</el-button>
      </el-col>
    </el-row>
  </el-form>
  <el-row class="tlw-row">
    <el-col :span="24">
      <el-table :data="tableData" border stripe>
        ...
        <el-table-column label="需归还信用指标">
          <el-table-column prop="" label="耕地数量（公顷）" />
          <el-table-column prop="" label="水田规模（公顷）" />
          <el-table-column prop="" label="粮食产能（公斤）" />
        </el-table-column>
        <el-table-column label="已归还信用指标">
          <el-table-column prop="" label="耕地数量（公顷）" />
          <el-table-column prop="" label="水田规模（公顷）" />
          <el-table-column prop="" label="粮食产能（公斤）" />
        </el-table-column>
        <el-table-column label="未归还信用指标">
          <el-table-column prop="" label="耕地数量（公顷）" />
          <el-table-column prop="" label="水田规模（公顷）" />
          <el-table-column prop="" label="粮食产能（公斤）" />
        </el-table-column>
        <el-table-column label="由省级需归还信用指标">
          <el-table-column prop="" label="耕地数量（公顷）" />
          <el-table-column prop="" label="水田规模（公顷）" />
          <el-table-column prop="" label="粮食产能（公斤）" />
        </el-table-column>
...
import { getTdzzGhzbtj } from "@/tdzz/api/zbgl/zbtj/ghzb/ghzb";
...
const F_getGhzbtjList = () => {
  getTdzzGhzbtj(data.queryParams).then((res) => {
